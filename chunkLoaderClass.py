    ...
    def load(self, chunk, chunk_pos):
        ...
        for x in range(len(chunk)):
            ...
            for y in range(len(chunk[x])):
                if chunk[x][y] == 0:
                    row.append(Air(self.block_size, x * self.block_size + self.chunk_size*chunk_pos[0]*self.block_size, y * self.block_size + self.chunk_size*chunk_pos[1]*self.block_size))
                elif chunk[x][y] == 1:
                    row.append(Dirt(self.block_size, x * self.block_size + self.chunk_size*chunk_pos[0]*self.block_size, y * self.block_size + self.chunk_size*chunk_pos[1]*self.block_size, self.images.get("dirt")))
